// *******************************************************************************************************
// **                                                                                                   **
// **   25LC020A.v - 25LC020A 2K-BIT SPI SERIAL EEPROM (VCC = +2.5V TO +5.5V)                           **
// **                                                                                                   **
// *******************************************************************************************************
// **                                                                                                   **
...
module M25LC020A (SI, SO, SCK, CS_N, WP_N, HOLD_N, RESET);
...
// *******************************************************************************************************
   ...
   reg  [07:00]         BaseAddress;                    // memory write base address
   reg  [07:00]         MemWrAddress;                   // memory write address
   reg  [07:00]         MemRdAddress;                   // memory read address
   ...
   always @(posedge CS_N) begin
      ...
      if ((BitCounter >= 24) & (BitCounter[2:0] == 0) & InstructionWRITE & WriteEnable & !WriteActive) begin
         for (LoopIndex = 0; LoopIndex < WriteCounter; LoopIndex = LoopIndex + 1) begin
            BaseAddress = {AddrRegister[7:4],4'h0};
            PageAddress = (AddrRegister[3:0] + LoopIndex);

            MemWrAddress = {BaseAddress[7:4],PageAddress[3:0]};

            if ({BlockProtect1,BlockProtect0} == 2'b00) begin
               WriteActive = 1;
            end
            if ({BlockProtect1,BlockProtect0} == 2'b01) begin
               if ((MemWrAddress >= 8'hC0) && (MemWrAddress <= 8'hFF)) begin
            ...
            if ({BlockProtect1,BlockProtect0} == 2'b10) begin
               if ((MemWrAddress >= 8'h80) && (MemWrAddress <= 8'hFF)) begin
                  // write protected region
               end
               else begin
                  WriteActive = 1;
            ...
            if ({BlockProtect1,BlockProtect0} == 2'b11) begin
               if ((MemWrAddress >= 8'h00) && (MemWrAddress <= 8'hFF)) begin
                  // write protected region
               end
               else begin
                  WriteActive = 1;
      ...
      if (HOLD_N == 1) begin
         if ((BitCounter >= 16) & (BitCounter[2:0] == 0) & InstructionREAD & !WriteActive) begin
            if (BitCounter == 16) begin
               DataShifterO <= MemoryBlock[AddrRegister[7:0]];
               MemRdAddress <= AddrRegister + 1;
               SO_Enable    <= 1;
            end
            else begin
               DataShifterO <= MemoryBlock[MemRdAddress[7:0]];
